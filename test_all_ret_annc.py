...
try:
    from . import test_ret_annc
    from .utils.lightning import fix_gpu_args
except ImportError:
    import test_ret_annc
    from utils.lightning import fix_gpu_args
...
if __name__ == "__main__":
    ...
    for model_seed_id in range(10):
        for language, dataset in language_dataset:
            ...
            # argument parsing
            str_args=f"-Vm {model_seed_id} -l {language} -d {dataset} --skip --max_epochs 20 --gpus=-1 -t 5000 -b 1" # 128
            parser = test_ret_annc.add_argparse_args()
            args = parser.parse_args(str_args.split())
            fix_gpu_args(args)

            print(f"Running for `{str_args}`")
            test_ret_annc.main(args)