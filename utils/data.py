...
def pad(tensor, bos_id, eos_id, pad_id=-1, target_size=-1):
    ...
    if target_size > 0 and tensor.size(-1) < target_size:
        tensor = F.pad(input=tensor, pad=(0,target_size - tensor.size(-1)), mode='constant', value=pad_id)

    return tensor

def collate(batch, bos_id, eos_id, pad_id=-1):
    ...
    for a, b, c, d in batch:
        a_emb.append(pad(a, bos_id, eos_id, pad_id, len_a+2))
        b_emb.append(pad(b, bos_id, eos_id, pad_id, len_b+2))
        c_emb.append(pad(c, bos_id, eos_id, pad_id, len_c+2))
        d_emb.append(pad(d, bos_id, eos_id, pad_id, len_d+2))
...
def collate_words(batch, bos_id, eos_id, pad_id):
    ...
    # make a tensor of all As, af all Bs, of all Cs and of all Ds
    a_emb = torch.stack([pad(a, bos_id, eos_id, pad_id, len_a+2) for a in batch])

    return a_emb

def prepare_dataset(dataset_year, language, nb_analogies_train, nb_analogies_val, nb_analogies_test, force_rebuild=False, split_seed=SPLIT_RANDOM_SEED, force_low=False, word_encoder="char"):
    ...
    if not force_low and ((dataset_year == "2019" and language in SIG2019_HIGH) or (dataset_year == "2016" and language=="japanese")):
        ...
        dataset = dataset_factory(dataset=dataset_year, language=language, mode=mode, word_encoder=word_encoder, force_rebuild=force_rebuild)
        ...
        dataset = dataset_factory(dataset=dataset_year, language=language, mode="train" if dataset_year == "2016" else "train-low", word_encoder=word_encoder, force_rebuild=force_rebuild)
        dataset_dev = dataset_factory(dataset=dataset_year, language=language, mode="dev", word_encoder=word_encoder, force_rebuild=force_rebuild)
        dataset_test = dataset_factory(dataset=dataset_year, language=language, mode="test", word_encoder=word_encoder, force_rebuild=force_rebuild)
...
def prepare_data(dataset_year, language, nb_analogies_train, nb_analogies_val, nb_analogies_test, batch_size = 32, force_rebuild=False, generator_seed=42, split_seed=SPLIT_RANDOM_SEED, force_low=False, pad_id=-1, word_encoder="char"):
    ...
    nb_analogies_val -- The number of analogies to use (before augmentation) for the validation (during the training).
    nb_analogies_test -- The number of analogies to use (before augmentation) for the testing (after the training).'''
    train_data, val_data, test_data, dataset = prepare_dataset(dataset_year, language, nb_analogies_train, nb_analogies_val, nb_analogies_test, force_rebuild=force_rebuild, split_seed=split_seed, force_low=force_low, word_encoder=word_encoder)
    # Load data
